...
- **Scorekeeper Focus** — from Admin, open the dedicated scoring workspace at
  `/adminp21/scorekeeper?tournamentId=…`. It provides grade/stage/status filters,
  touch-friendly score inputs, preliminary/finals/redemption scoring, extra-time and
  sudden-death controls, save feedback, and Firebase connection status.
- **Export & restore** — Admin → Export & restore downloads a complete, versioned JSON
  snapshot. Selecting a backup validates and previews its source before replacing the
  current tournament ID.
- **Reports** — the same panel exports teams and matches as CSV, plus a printable HTML
  results sheet that can be printed or saved as PDF from a browser.
...
- `tournaments/{tournamentId}/teams/{teamId}` — `{ gradeId, divisionId, name, code?, schoolId?, fairPlayPoints?, japanCupChampionOnly? }` — optional `schoolId` for display. **`fairPlayPoints` on the team** is a cached sum of student balances (maintained on write); standings calculate a roster-size-neutral percentage from student balances. Legacy team-only values are converted from the former 15-point scale until students are initialized. **`japanCupChampionOnly: true`** marks the Japan Cup defending champion (deterministic id `{gradeId}__JC`); registered separately in admin, not in Teams A/B, and excluded from brackets.
- `tournaments/{tournamentId}/students/{studentId}` — `{ name, teamId?, fairPlayPoints?, fairPlayInitialShare?, japanCupEligible? }` — roster required for Fair Play (within-school). **`teamId`** is always the canonical Firebase team key; in admin you may enter a team **`code`** (e.g. `G1-A-01`) or bulk CSV `studentId,name,teamCode` and the app resolves it before save. Every student starts with **15 personal points**. **`japanCupEligible`** is set when the finals bracket is generated for that grade (`fairPlayPoints > 0` at lock). Preliminary standings sort by match points first, then team Fair Play percentage; Fair Play does not alter finals match scores.
